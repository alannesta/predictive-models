...
def create_predictions_df(train_x, train_y, test_x, production_df, game_info_df):
    ...
    odds = 1 / predictions
    merged_cols = ['Date', 'HomeTeam', 'AwayTeam', 'gameId', 'homeModelledOdds', 'drawModelledOdds', 'awayModelledOdds',
                   'eloHome', 'eloAway']
    print(predictions)
    print(game_info_df)
    print(production_df)
    ...
    predictions_df = (production_df.assign(homeModelledOdds=[i[2] for i in (1 / predictions)],
                      ...
                      .pipe(
        lambda df: pd.merge(df[merged_cols], game_info_df.drop(columns='Date'), on=['HomeTeam', 'AwayTeam'])))

    print(predictions_df)
    # Assign the modelled winner to our predictions df
    predictions_df = (predictions_df.assign(
        favourite=lambda df: df[['homeModelledOdds', 'drawModelledOdds', 'awayModelledOdds']].idxmin(axis=1))
                      .assign(modelledWinner=lambda df: df.apply(lambda row:
                                                                 row.HomeTeam if row.favourite == 'homeModelledOdds' else (
                                                                     row.AwayTeam if row.favourite == 'awayModelledOdds' else 'The Draw'),
                                                                 axis=1)))
