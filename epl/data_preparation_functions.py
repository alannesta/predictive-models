    ...
    df = (pd.read_csv(path, dtype={'season': str})
          ...
          .assign(gameId=lambda df: list(df.index + 1),
                  ...
                  result=lambda df: df.apply(
                      lambda row: 'home' if row.FTHG > row.FTAG else ('draw' if row.FTHG == row.FTAG else 'away'),
                      axis=1)))
    return df
...
def create_multiline_df_stats(old_stats_df):
    ...
    home_stats_cols = ['HomeTeam', 'FTHG', 'FTAG', 'HTHG', 'HTAG', 'HS', 'AS', 'HST', 'AST', 'HF', 'AF', 'HC', 'AC',
                       'HY', 'AY',
                       'HR', 'AR']
    away_stats_cols = ['AwayTeam', 'FTAG', 'FTHG', 'HTAG', 'HTHG', 'AS', 'HS', 'AST', 'HST', 'AF', 'HF', 'AC', 'HC',
                       'AY', 'HY',
                       'AR', 'HR']
...
def create_all_games_features(all_games):
    ...
    elos, elo_probs, all_elos_current = elo_applier(df=all_games,
                                                    k_factor=25,
                                                    historic_elos={team: 1500 for team in
                                                                   all_games['HomeTeam'].unique()},
    ...
    multi_line_all_games['wtEloGoalsFor'] = [
        wt_goals_elo(df=multi_line_all_games,
                     game_id_row=row[multi_line_all_games.columns.get_loc('gameIdHistoric') + 1],
                     # Get the index of 'gameIdHistoric'
                     team_row=row[multi_line_all_games.columns.get_loc('team') + 1],
                     # Get the index of 'gameIdHistoric'
...
def put_features_on_one_line(features_multi_line):
    ...
    features = (features_multi_line[features_multi_line.homeGame == 1]
                .rename(
        columns={col: 'f_' + col + 'Home' for col in features_multi_line.columns if col not in non_features})
...
def create_market_values_features(df):
    ...
    def_vals = values[values['Position'] == 'Defender'].groupby(['TeamName', 'Year'], as_index=False)[
        'marketValueMillions'].sum().rename(columns={'marketValueMillions': 'defMktVal'})
    att_vals = values[values['Position'] == 'Forward'].groupby(['TeamName', 'Year'], as_index=False)[
        'marketValueMillions'].sum().rename(columns={'marketValueMillions': 'attMktVal'})
    gk_vals = values[values['Position'] == 'Goalkeeper'].groupby(['TeamName', 'Year'], as_index=False)[
        'marketValueMillions'].sum().rename(columns={'marketValueMillions': 'gkMktVal'})
    total_vals = values[values['Position'] == 'Total:'].groupby(['TeamName', 'Year'], as_index=False)[
        'marketValueMillions'].sum().rename(columns={'marketValueMillions': 'totalMktVal'})
    final_vals = reduce(lambda left, right: pd.merge(left, right, on=['TeamName', 'Year'], how='inner'),
                        [def_vals, att_vals, gk_vals, total_vals])
    final_vals['midMktVal'] = final_vals.totalMktVal - final_vals.defMktVal - final_vals.attMktVal - final_vals.gkMktVal

    mkt_vals_joined = pd.merge(df[['gameId', 'Year', 'HomeTeam', 'AwayTeam']],
                               final_vals.rename(columns={'TeamName': 'HomeTeam'}), on=['HomeTeam', 'Year'], how='left')
    mkt_vals_joined = pd.merge(mkt_vals_joined, final_vals.rename(columns={'TeamName': 'AwayTeam'}),
                               on=['AwayTeam', 'Year'], how='left', suffixes=('H', 'A'))

    for col in ['attMktVal', 'midMktVal', 'defMktVal', 'gkMktVal', 'totalMktVal']:
        mkt_vals_yrlyH = mkt_vals_joined.groupby(['Year', 'HomeTeam'])[col + 'H'].first().groupby(level=0).apply(
            lambda x: x / x.sum()).unstack()
        mkt_vals_yrlyA = mkt_vals_joined.groupby(['Year', 'AwayTeam'])[col + 'A'].first().groupby(level=0).apply(
            lambda x: x / x.sum()).unstack()
        mkt_vals_joined[col[:-3] + 'H%'] = mkt_vals_joined.apply(
            lambda x: mkt_vals_yrlyH.to_dict()[x['HomeTeam']][x['Year']] * 100, axis=1)
        mkt_vals_joined[col[:-3] + 'A%'] = mkt_vals_joined.apply(
            lambda x: mkt_vals_yrlyA.to_dict()[x['AwayTeam']][x['Year']] * 100, axis=1)
...
def create_feature_df(df=None):
    ...
    all_games_cols = ['Date', 'gameId', 'team', 'season', 'homeGame', 'homeWinPc38', 'homeWinPc5', 'awayWinPc38',
                      'awayWinPc5', 'eloFor', 'eloAgainst', 'wtEloGoalsFor', 'wtEloGoalsAgainst']
    ...
    market_val_feature_names = ['attMktH%', 'attMktA%', 'midMktH%', 'midMktA%', 'defMktH%', 'defMktA%', 'gkMktH%',
                                'gkMktA%', 'totalMktH%', 'totalMktA%']
    ...
    features_with_na = (features_with_na.pipe(pd.merge,
                                              features_market_values[market_val_feature_names + ['gameId']].rename(
                                                  columns={col: 'f_' + col for col in market_val_feature_names}),
                                              on='gameId')
                        .pipe(pd.merge, df[['HomeTeam', 'AwayTeam', 'gameId', 'result', 'B365A', 'B365D', 'B365H']],
                              on=['HomeTeam', 'AwayTeam', 'gameId'])
                        .drop(columns='homeGame'))

    # Drop NAs from calculating the rolling averages - don't drop Win Pc 38 and Win Pc 5 columns
    features = features_with_na.dropna(
        subset=features_with_na.drop(columns=[col for col in features_with_na.columns if 'WinPc' in col]).columns)
